...
class TextRank:
    ...
    def readTagger(self):
        original_content = preprocess(self.content)
        # split content using regular expression
    ...
    def numeric(self, sent):
        ...
        for idx, word in enumerate(words):
            ...
            # number + noun
            if prev_word[1] == 'SN' and (word[1] == 'NNP' or word[1] == 'NNG' or word[1] == 'SL'):
                if prev_word[0] + word[0] in self.content:
                    words[idx] = (prev_word[0] + word[0], word[1])
                elif prev_word[0] + ' ' + word[0] in self.content:
                    words[idx] = (prev_word[0] + ' ' + word[0], word[1])
            '''
            # noun + number
            if (prev_word[1] == 'NNP' or prev_word[1] == 'NNG' or prev_word[1] == 'SL') and word[1] == 'SN':
                if prev_word[0] + word[0] in self.content:
                    words[idx] = (prev_word[0] + word[0], prev_word[1])
                    print(words[idx])
                elif prev_word[0] + ' ' + word[0] in self.content:
                    words[idx] = (prev_word[0] + ' ' + word[0], prev_word[1])
                    print(words[idx])
    ...
    def __init__(self, **kwargs):
        ...
        self.taggerTokenizer = lambda x: x[0] not in self.stopwords and x[1] in ('NNG', 'NNP', 'SL') and (len(x[0]) > 1 or x[0] in self.singlewords) and re.compile('[一-龥]+').match(x[0]) is None
